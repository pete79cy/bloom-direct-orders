...
**Date:** 2026-06-25
**Repo:** bloom-direct-orders (PWA). No bloom-crm change — reuses the existing `POST /api/customers`.
...
| iOS mechanism | Apple Shortcut → deep link `/customers/new?name=…&phone=…`. |
| Fields carried | name + phone. (email = out of scope for v1.) |
...
iPhone Contacts → Share → "Add to Bloom" (Apple Shortcut, built once)
   ...
   ▼
Opens (in Safari): https://orders.smartquotations.eu/customers/new?name=…&phone=…
   │  RequireAuth — log in once in Safari if needed (Remember me)
   ▼
AddCustomerPage: new-customer form PRE-FILLED (name → trading_name, phone → phone)
...
- Reads `name` and `phone` from `useSearchParams()`.
- Renders the new-customer form pre-filled. Reuses `useCreateCustomer` and the
  ...
  phone, payment_terms_days defaulting to '0'). To stay DRY, extract the small
  presentational `Field` input from `NewCustomerSheet.tsx` into a shared
  ...
  targeted, in-scope improvement).
- Pre-fill mapping: `name` → `tradingName`, `phone` → `phone`. Empty/missing
  params → empty fields (form still usable).
...
2. Gets the contact's **Name** and **Phone Number** (first phone).
3. URL-encodes each.
4. Builds the URL `https://orders.smartquotations.eu/customers/new?name=<name>&phone=<phone>`.
5. **Open URLs**.
...
- No email carried from the contact (name + phone only).
...
## Testing
- `AddCustomerPage`: given `?name=Foo&phone=99123456`, the trading-name field
  shows "Foo" and the phone field shows "99123456" (render test with a
  MemoryRouter + initial entry).
- `AddCustomerPage`: empty params → empty fields, Save disabled until a trading
  name is entered.
- Save calls `useCreateCustomer` with `{ trading_name, phone, payment_terms_days: 0 }`
